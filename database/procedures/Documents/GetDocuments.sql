...
CREATE OR ALTER PROCEDURE [dbo].[SP_Documents_GetDocuments]
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF dbo.CheckUserRole(@IN_currentUserId, 'Professor') = 0 
           AND dbo.CheckUserRole(@IN_currentUserId, 'Student') = 0
        BEGIN
        ...
        BEGIN
            ...
            SET @IN_pageSize = (SELECT COUNT(*) 
                                FROM [dbo].[CourseDocument] CD 
                                JOIN [dbo].[Document] D ON CD.documentId = D.documentId 
                                WHERE CD.courseId = @IN_courseId AND D.isDeleted IS NULL);
        END;
