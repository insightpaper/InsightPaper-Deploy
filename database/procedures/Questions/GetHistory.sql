...
CREATE OR ALTER     PROCEDURE [dbo].[SP_Questions_GetHistory]
    -- Parameters
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF dbo.CheckUserRole(@IN_currentUserId, 'Student') = 0 AND dbo.CheckUserRole(@IN_currentUserId, 'Professor') = 0
        BEGIN
        ...
        SELECT 
			...
			(
				...
				WHERE C.questionId = Q.questionId
				  AND (C.isPrivate = 0 OR C.isPrivate IS NULL)
				FOR JSON PATH
			) AS comments
		FROM 
			[dbo].[Question] AS Q
