...
-- Date:         2025-06-01
-- Description:  Take all notificacions.
--------------------------------------------------------------------------

CREATE OR ALTER   PROCEDURE [dbo].[SP_Users_SelectCourseNotifications]
    -- Parameters
...
BEGIN
    ...
    BEGIN TRY
		...
		JOIN [dbo].[Users] AS U ON U.userId = N.userId
		WHERE DATEDIFF(SECOND, N.createdDate, GETUTCDATE()) < 60
		  AND N.isDeleted != 1
		  AND N.objectNotificatedId = @IN_courseId
		  AND U.isDeleted != 1
